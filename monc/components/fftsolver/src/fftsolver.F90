...
module fftsolver_mod
  ...
  ! filter for immersed boundary
  logical :: filter_enabled     ! activate filter
  real(kind=DEFAULT_PRECISION), dimension(:,:,:), allocatable :: tmp_filt
  real(kind=DEFAULT_PRECISION), dimension(:,:,:), allocatable :: pf
...
contains
  ...
  subroutine initialisation_callback(current_state)
    ...
    if (current_state%immersed%ib_enabled) then
      l_tmp=options_get_logical(current_state%options_database, "filter_enabled")
      if(l_tmp)filter_enabled=options_get_logical(current_state%options_database, "p_filter_enabled")
      ...
      if (filter_enabled) then
        ...
        allocate(pf(fourier_space_sizes(Z_INDEX), fourier_space_sizes(Y_INDEX), fourier_space_sizes(X_INDEX)))
    ...
    if (filter_enabled) then
      ...
      do ipi=1,fourier_space_sizes(X_INDEX)
        do ipj=1,fourier_space_sizes(Y_INDEX)
          do ipk=1,kmax
            pf(ipk,ipj,ipi) = pt(ipk,ipj,ipi)*filter_coefs(ipj, ipi)
          end do
        end do
      end do
     
    ! back to real space and merge fields
      call perform_backwards_3dfft(current_state, pf, tmp_filt(start_loc(Z_INDEX):end_loc(Z_INDEX), &
             start_loc(Y_INDEX):end_loc(Y_INDEX), start_loc(X_INDEX):end_loc(X_INDEX)))   
        do ipi=start_loc(X_INDEX),end_loc(X_INDEX)
          do ipj=start_loc(Y_INDEX),end_loc(Y_INDEX)
            do ipk=1,kmax
              current_state%p%data(ipk,ipj,ipi) = current_state%p%data(ipk,ipj,ipi)*current_state%immersed%ibprox(ipk,ipj,ipi) +&
                                      tmp_filt(ipk,ipj,ipi)*(1.0_DEFAULT_PRECISION-current_state%immersed%ibprox(ipk,ipj,ipi))
            end do
          end do
        end do
        current_state%p%data = tmp_filt
      end if ! end filter
